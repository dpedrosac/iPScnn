...
class Filter:
    ...
    def Q_jacobian(self, x, signal, t):
    ...
    def multi_notch(self, series, window, notch_frequencies):
        ...
        for freq in notch_frequencies:
            ...
            for val in windows_strided:
                ...
                res = minimize(Q_x, x_est, args=(val, t), method='L-BFGS-B', bounds=bounds, jac=self.Q_jacobian,
    ...
    def apply_filter(self, df: pd.DataFrame):
        ...
        for channel_name in columns:
            ...
            df[channel_name] = self.pre_process(df[channel_name])
        print('', flush=True)
