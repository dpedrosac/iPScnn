...
shallow.emg.agg.pickle <- function(fname, lpar, details)
...
{
    df  = read.pickle(fname)        
    dfm = data.frame(t(base::colMeans(df)))
    ...
    if(dfm$cond == "OFF"){
      dfm$updrs = details$updrsOFF[ details$Name == dfm$subid]
    }else{
      dfm$updrs = details$updrsON[  details$Name == dfm$subid]
    }
return(dfm)
}
